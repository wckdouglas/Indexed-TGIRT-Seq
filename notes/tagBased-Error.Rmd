...
* $\hat{\theta}_{null}$ be $\hat{\theta}$ without $\theta_{max}$

likelihood ratio test was performed by:
\[
log(\Lambda) = log(max(\hat{\theta})) - logsumexp(\hat{\theta}_{null})
\]
...
p <- ggplot(data=df, aes(y= loglik, x=ratio)) +
        geom_point(alpha=0.7,aes(color=cov,size=count)) +
        scale_x_continuous(breaks=seq(0,1,0.05)) +
        scale_y_continuous(breaks=seq(0,5,0.5)) +
        theme(axis.text.x=element_text(angle=90,hjust=1,vjust=0.5)) +
        geom_smooth(method='lm') +
        geom_vline(x=0.95, color='green') +
        scale_size_continuous(breaks=seq(0,300,50))+
        scale_color_gradient(high='red',low='yellow') +
        labs(x='Correct base ratio',y = 'Normalized log likelihood ratio', color='Base coverage', size = '# of events')
p
```
