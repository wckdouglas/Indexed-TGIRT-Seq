...
\[
P(I=b | r_{j},Q_{j}) = \frac{\prod_i (P(I=b|r_{j},Q_{j}))}{\sum_{d \in \{A,C,T,G\}}\prod_i (P(I=d|r_{j},Q_{j}))}
\]

For the following example illutrated with **R**, if a position in a read cluster contains bad sequencing reads [C, C, C, C, A, A, A, C] with Phred scores [54, 54, 54, 33, 80, 80, 80, 42]:

```{r warning = F, message = F }
library(cowplot)

q <- c(54, 54, 54, 33, 80, 80, 80, 42)
b <- c('C', 'C', 'C', 'C', 'A', 'A', 'A', 'C')
...
qplot(bases, posteriorsDist) + 
  ...
  labs(x = ' hi', y = 'Posterior Probability')
...
\[
Q = 10log_{10} (P(I=C | r_{j},Q_{j}))
\]

```{r, echo =F }
Quality = - 10 * log10(1 - posteriorsDist[bases=='C'])
```

In the above example, uncorrected quality score would be `r Quality`.

![](/Users/wckdouglas/labWork/projects/plasmaDNA_project/manuscripts/figures/indexed_tgirt.pdf)